...
const maxDistance = adjustedScanners.reduce((acc, dis) => {
    adjustedScanners.forEach(otter => {
            let manhattanD = zip(dis.position, otter.position).map(d => Math.abs(d[0] - d[1])).reduce(add, 0)
            acc = Math.max(acc, manhattanD)
    });
    return acc;
}, 0);
