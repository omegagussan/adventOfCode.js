...
function updateMap(map, key, value){
    let old = map.get(key)
    let val = old !== undefined ? old + value: value
    map.set(key, val)
}
...
function initCubes(lines) {
    ...
    for (let line of lines) {
        ...
        console.log(newCubeRanges)
        if (newCubeRanges.some(e => Math.abs(e) > 50)) continue
        let updates = new Map()
        let tmp = [...cubes.entries()]
        console.log('t', tmp)
        tmp = tmp.reverse()
        console.log('tr', tmp)
        for (let [oldKey, oldStatus] of tmp){
            ...
            if (dimensionWiseComparison.every(d => d[1] >= d[0])) {
                updateMap(updates, akey(dimensionWiseComparison.flat()), oldStatus === 0 ? oldStatus : -oldStatus)
            }
        }
...
function countLitCubes(cubes) {
    ...
    for (let [key, sign] of cubes.entries()) {
        ...
        if (sign !== 0) sum += (dimensions[0] + 1) * (dimensions[1] + 1) * (dimensions[2] + 1) * sign
    }
    return sum;
}
